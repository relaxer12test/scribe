defmodule Ueberauth.Strategy.Salesforce.OAuth do
  ...
  def get_access_token(params \\ [], opts \\ []) do
    ...
    case opts |> client() |> OAuth2.Client.get_token(params) do
      ...
      {:error, %OAuth2.Response{body: body}} ->
        {:error, {"oauth_error", inspect(body)}}
  ...
  """
  def get_user(%OAuth2.AccessToken{} = token) do
    url = token.other_params["id"] || user_info_url()

    case Tesla.get(http_client(token), url) do
  ...
  defp user_info_url do
    config = Application.get_env(:ueberauth, __MODULE__, [])
    site = Keyword.get(config, :site, @defaults[:site])
    site <> "/services/oauth2/userinfo"
  end

  defp http_client(%OAuth2.AccessToken{} = token) do
    Tesla.client([
